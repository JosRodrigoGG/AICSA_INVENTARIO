		...
		var drawHead = function () {
			...
			/* Draw table */
				...
				for ( iCol in self.options.cols ) {
					...
					).addClass( "ui-state-default ui-pivot-item ui-pivot-droppable" ).attr("class", "TITULO_AGRUPADOR_COLUMNA");
				...
				for ( iRow in self.options.rows ) {
					...
					th = $col.clone().html( self.options.rows[ iRow ] + sortIcon ).addClass( "ui-state-default ui-pivot-item ui-pivot-droppable" ).attr("class", "TITULO_ENCABEZADO_COLUMNA");
		...
		var drawColTitles = function () {
			...
			do {
				...
				for ( i = 0; i < ptr.sorted.length; ++i ) {
					...
					dim = $dim.clone().addClass( "ui-state-default" ).html( ptr.items[ item ].value === null ? "" : ptr.items[ item ].value + sortIcon ).addClass('ENCABEZADO_NOMBRE');
