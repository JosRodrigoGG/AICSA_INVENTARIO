...
,p_last_upd_yyyymmddhh24miss=>'20231009111049'
);
...
'        WHEN :P124_TIPO_INDICADOR = 17 THEN',
'            TO_CHAR(',
...
'                            (',
'                                SELECT ',
'                                    REPLACE(REPLACE(REGEXP_SUBSTR(:P124_UNIDAD, ''[^\|]+'', 1, level), ''['',''''), '']'', '''') AS CODIGO_DIVISION',
'                                FROM ',
'                                    dual CONNECT BY REGEXP_SUBSTR(:P124_UNIDAD, ''[^\|]+'', 1, level) IS NOT NULL',
'                            )',
...
'                                        dual CONNECT BY REGEXP_SUBSTR(:P124_UNIDAD, ''[^\|]+'', 1, level) IS NOT NULL',
'                                )',
'                				WHERE A.CODIGO_DIVISION = TO_NUMBER(CODIGO_DIVISION)',
'                			) > 0 THEN',
'                            ''A''',
'                        ELSE',
'                            NULL',
...
'            (',
'                SELECT CODIGO_DIVISION FROM TEMP_UNIDAD WHERE ESTADO IS NOT NULL',
'            )',
...
'            NULL AS CODIGO,',
'    		NULL AS NOMBRE',
'        FROM SAF.CON_INTEGRACION_GERENCIAL ',
'        WHERE COD_CTA LIKE ''11%''',
'        AND USUARIO_GRABACION = 1;',
'    ',
'    CURSOR C_EGRESO IS',
'        SELECT ',
'            DISTINCT COD_CTA AS PADRE,',
