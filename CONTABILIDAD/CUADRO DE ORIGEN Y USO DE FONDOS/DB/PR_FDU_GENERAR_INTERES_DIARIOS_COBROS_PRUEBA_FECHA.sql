CREATE OR REPLACE PROCEDURE SAF.PR_FDU_GENERAR_INTERES_DIARIOS_COBROS_PRUEBA_FECHA (V_FECHA_CALCULAR DATE) IS
    CURSOR C_DATOS IS
        SELECT
            A.ID
    ...
    LOOP
        ...
        (
            SELECT
                ...
                A.TASA_CAMBIO,
                A.CODIGO_MONEDA,
                V_FECHA_CALCULAR AS FECHA_COBRO
            FROM SAF.FDU_TRANSACCIONES_FINANCIERAS A
            WHERE A.ID = R_DATOS.ID
            AND TRUNC(V_FECHA_CALCULAR) NOT IN
            (
        ...
        IF V_ID IS NOT NULL THEN
            IF EXTRACT(DAY FROM V_FECHA_CALCULAR) = (EXTRACT(DAY FROM LAST_DAY(V_FECHA_CALCULAR)) - 1) THEN
                INSERT INTO SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_DETALLE
                (
                    ID_FDU_TRANSACCIONES_FINANCIERAS,
                    FECHA_INTERES,
                ...
                (
                    SELECT
                        ...
                        A.TASA_CAMBIO,
                        A.CODIGO_MONEDA,
                        (V_FECHA_CALCULAR + 1) AS FECHA_COBRO
                    FROM SAF.FDU_TRANSACCIONES_FINANCIERAS A
                    WHERE A.ID = R_DATOS.ID
                    AND TRUNC(V_FECHA_CALCULAR + 1) NOT IN
                    (
                        SELECT TRUNC(FECHA_INTERES) FROM SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_DETALLE WHERE ID_FDU_TRANSACCIONES_FINANCIERAS = R_DATOS.ID
                    )
                );
                ...
                SELECT
                    ...
                    TO_NUMBER(TO_CHAR(V_FECHA_CALCULAR, 'MM'))  MES,
                    TO_CHAR(V_FECHA_CALCULAR, 'YYYY') ANIO,
                    1,
                    1 USUARIO_GRABACION,
                ...
                AND TRUNC(C.FECHA_INTERES) <= TRUNC(V_FECHA_CALCULAR + 1)
                --AND TO_NUMBER(TO_CHAR(C.FECHA_INTERES, 'MM')) <= TO_NUMBER(TO_CHAR(V_FECHA_CALCULAR, 'MM'))
                --AND TO_NUMBER(TO_CHAR(C.FECHA_INTERES, 'YYYY')) <= TO_NUMBER(TO_CHAR(V_FECHA_CALCULAR, 'YYYY'))
                AND C.ESTADO = 1
                ...
                AND A.MES = TO_NUMBER(TO_CHAR(V_FECHA_CALCULAR, 'MM'))
                AND TO_NUMBER(A.ANIO) = TO_NUMBER(TO_CHAR(V_FECHA_CALCULAR, 'YYYY'));
            ...
            IF EXTRACT(DAY FROM V_FECHA_CALCULAR) = 1 THEN
                DECLARE
                    CURSOR C_CAPITAL IS
                        SELECT
                            A.ESTADO
                        FROM SAF.FDU_TRANSACCIONES_FINANCIERAS_PAGOS_CAPITAL A
                        WHERE A.MES = EXTRACT(MONTH FROM (V_FECHA_CALCULAR - 1))
                        AND A.ANIO = EXTRACT(YEAR FROM (V_FECHA_CALCULAR - 1))
                    ...
                    CURSOR C_INTERES IS
                        ...
                        WHERE B.MES = EXTRACT(MONTH FROM (V_FECHA_CALCULAR - 1))
                        AND B.ANIO = EXTRACT(YEAR FROM (V_FECHA_CALCULAR - 1))
                    ...
                    IF V_ESTADO_CAPITAL != 3 AND V_ESTADO_INTERES != 3 THEN
                        ...
                        IF V_TIPO_COBRO_MORA = 1 THEN
                            IF V_TIPO_MORA = 1 THEN
                                ...
                                FROM SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_COBRADOS A
                                WHERE A.ID_FDU_TRANSACCIONES_FINANCIERAS = R_DATOS.ID
                                AND A.MES = EXTRACT(MONTH FROM (V_FECHA_CALCULAR - 1))
                                AND A.ANIO = EXTRACT(YEAR FROM (V_FECHA_CALCULAR - 1))
                                AND A.ESTADO != 3;
                                ...
                                AND B.MES = EXTRACT(MONTH FROM (V_FECHA_CALCULAR - 1))
                                AND B.ANIO = EXTRACT(YEAR FROM (V_FECHA_CALCULAR - 1))
                                AND B.ESTADO != 3
                                AND B.TIPO_PAGO = 1;
                                ...
                                SELECT
                                    ...
                                    EXTRACT(MONTH FROM (V_FECHA_CALCULAR - 1)),
                                    EXTRACT(YEAR FROM (V_FECHA_CALCULAR - 1)),
                                    1,
                                    1,
                                    SYSDATE
                        ...
                        ELSE
                            ...
                            IF V_FECHA_ACEPTACION = V_FECHA_CALCULAR THEN
                                IF V_TIPO_MORA = 1 THEN
                                    INSERT INTO SAF.FDU_TRANSACCIONES_FINANCIERAS_MORAS
                                    (
                                        ID_FDU_TRANSACCIONES_FINANCIERAS,
                                    ...
                                    SELECT
                                        ...
                                        EXTRACT(MONTH FROM V_FECHA_CALCULAR),
                                        EXTRACT(YEAR FROM V_FECHA_CALCULAR),
                                        1,
                                        1,
                                        SYSDATE
                                    FROM SAF.FDU_TRANSACCIONES_FINANCIERAS A
