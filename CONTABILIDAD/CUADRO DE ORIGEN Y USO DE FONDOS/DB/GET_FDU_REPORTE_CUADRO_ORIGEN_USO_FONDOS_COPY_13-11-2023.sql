...
RETURN FDU_T_CUADRO_ORIGEN_USO_FONDO AS
    ...
    CURSOR C_INGRESOS_EGRESOS(V_MONEDA NUMBER, V_FECHA_INICIO DATE, V_FECHA_FIN DATE, V_EMPRESAS VARCHAR2) IS
        ...
        (
            ...
            AND TRUNC(SAF.FNC_GET_FECHA_TRSABONO(A.CODIGO_PROVEEDOR, C.TIPO_TRANSACCION_ABONO, C.SERIE_DOCUMENTO_ABONO, C.NUMERO_DOCUMENTO_ABONO)) BETWEEN TRUNC(TO_DATE(V_FECHA_INICIO, 'DD/MM/YYYY')) AND TRUNC(TO_DATE(V_FECHA_FIN, 'DD/MM/YYYY'))
        ...
        SELECT
            NVL(SAF.FNC_GET_NOMBRE_CLIENTE(CODIGO_EMPRESA, NUMERO_CUENTA), CODIGO_EMPRESA || ' - Otros Registros (' || NUMERO_CUENTA || ')') DESCRIPCION,
            NUMERO_CUENTA,
            NVL(SUM(VALOR_INGRESO), 0) VALOR_INGRESO,
            NVL(SUM(VALOR_EGRESO), 0) VALOR_EGRESO
        FROM VALORES
        GROUP BY NUMERO_CUENTA, NVL(SAF.FNC_GET_NOMBRE_CLIENTE(CODIGO_EMPRESA, NUMERO_CUENTA), CODIGO_EMPRESA || ' - Otros Registros (' || NUMERO_CUENTA || ')');
