CREATE OR REPLACE PROCEDURE SAF.PR_FDU_GENERAR_INTERES_DIARIOS_COBROS IS
    CURSOR C_DATOS IS
        SELECT
            A.ID
...
BEGIN
    ...
    LOOP
        ...
        IF V_ID IS NOT NULL THEN
            IF EXTRACT(DAY FROM SYSDATE) = (EXTRACT(DAY FROM LAST_DAY(SYSDATE)) - 1) THEN
                INSERT INTO SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_DETALLE
                (
                    ID_FDU_TRANSACCIONES_FINANCIERAS,
                    FECHA_INTERES,
                ...
                (
                    SELECT
                        ...
                        A.TASA_CAMBIO,
                        A.CODIGO_MONEDA,
                        (SYSDATE + 1) AS FECHA_COBRO
                    FROM SAF.FDU_TRANSACCIONES_FINANCIERAS A
                    WHERE A.ID = R_DATOS.ID
                    AND TRUNC(SYSDATE + 1) NOT IN
                    (
                        SELECT TRUNC(FECHA_INTERES) FROM SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_DETALLE WHERE ID_FDU_TRANSACCIONES_FINANCIERAS = R_DATOS.ID
                    )
                );
                ...
                SELECT
                    ...
                    TO_NUMBER(TO_CHAR(SYSDATE, 'MM'))  MES,
                ...
                AND TRUNC(C.FECHA_INTERES) <= TRUNC(SYSDATE + 1)
                AND C.ESTADO = 1
                ...
                AND A.MES = TO_NUMBER(TO_CHAR(SYSDATE, 'MM'))
                AND TO_NUMBER(A.ANIO) = TO_NUMBER(TO_CHAR(SYSDATE, 'YYYY'));
            ...
            IF EXTRACT(DAY FROM SYSDATE) = 1 THEN
                DECLARE
                    CURSOR C_CAPITAL IS
                        ...
                        WHERE A.MES = EXTRACT(MONTH FROM (SYSDATE - 1))
                        AND A.ANIO = EXTRACT(YEAR FROM (SYSDATE - 1))
                    ...
                    CURSOR C_INTERES IS
                        ...
                        WHERE B.MES = EXTRACT(MONTH FROM (SYSDATE - 1))
                        AND B.ANIO = EXTRACT(YEAR FROM (SYSDATE - 1))
                    ...
                    IF V_ESTADO_CAPITAL != 3 AND V_ESTADO_INTERES != 3 THEN
                        ...
                        IF V_TIPO_COBRO_MORA = 1 THEN
                            IF V_TIPO_MORA = 1 THEN
                                ...
                                FROM SAF.FDU_TRANSACCIONES_FINANCIERAS_INTERES_COBRADOS A
                                WHERE A.ID_FDU_TRANSACCIONES_FINANCIERAS = R_DATOS.ID
                                AND A.MES = EXTRACT(MONTH FROM (SYSDATE - 1))
                                AND A.ANIO = EXTRACT(YEAR FROM (SYSDATE - 1))
                                AND A.ESTADO != 3;
                                ...
                                AND B.MES = EXTRACT(MONTH FROM (SYSDATE - 1))
                                AND B.ANIO = EXTRACT(YEAR FROM (SYSDATE - 1))
                                AND B.ESTADO != 3
                                AND B.TIPO_PAGO = 1;
                        ...
                        ELSE
                            ...
                            IF V_FECHA_ACEPTACION = SYSDATE THEN
                                IF V_TIPO_MORA = 1 THEN
                                    INSERT INTO SAF.FDU_TRANSACCIONES_FINANCIERAS_MORAS
                                    (
                                        ID_FDU_TRANSACCIONES_FINANCIERAS,
